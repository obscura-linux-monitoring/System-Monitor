...
struct CommandResult
{
    ...
    int commandStatus;    ///< 커맨드 작업 유형
    int resultStatus;     ///< 커맨드 작업 결과 상태
    string resultMessage; ///< 처리 결과 메시지
    string target;        ///< 커맨드 대상

    // 기본 생성자
    CommandResult() : commandID(0), commandStatus(0), resultStatus(0) {}

    // 매개변수가 있는 생성자
    CommandResult(int id, const string &node, const string &type,
                  int status, int result, const string &message, const string &tgt)
        : commandID(id), nodeID(node), commandType(type),
          commandStatus(status), resultStatus(result), resultMessage(message), target(tgt) {}
};
